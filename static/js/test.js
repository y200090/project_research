...
async function main() {
    ...
    async function checkAnswer(input) {
        ...
        if (userAnswer[index] == answer[index]) {
            answerImage = '<i class="bx bx-check-circle answer-image correct result"></i>';
            ...
            answerImage = '<i class="bx bx-x-circle answer-image incorrect result"></i>';
            ...
            const modelAnswerImage = '<i class="bx bx-check-circle answer-image correct result"></i>';
        ...
        // テストページのクローンを取得
        clone[index] = testPage.cloneNode(true);
        // セッションストレージにクローンを保存
        sessionStorage.setItem(`test-${index + 1}`, `${clone[index].innerHTML}`);
